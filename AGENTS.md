...
- `cargo run -- --pretty projects` — pretty-print JSON output for any read command.
...
- `cargo run -- messages --session sess-123` — messages for a specific session; without `--project`, searches all projects and bypasses cwd auto-discovery.
...
- `mmr export --project <path>` passes the project to a single `messages` call (all sources unless `--source` is set) with no page limit. Reuses existing `ApiMessagesResponse`; no new response type.
...
- `mmr export` is intentionally unpaginated: `next_page` is always `false`, `next_offset` equals `total_messages`, and `next_command` is omitted.
- Scripts that need only the message array can pipe through `jq '.messages'`.

## CLI default env vars

- `MMR_AUTO_DISCOVER_PROJECT=0` disables cwd project auto-discovery for `sessions` and `messages`; any other value (including unset or `1`) keeps the default auto-discovery behavior.
...
## Messages response and pagination

- `ApiMessagesResponse` contains `messages`, `total_messages`, `next_page`, `next_offset`, and optional `next_command`.
- For the default `messages` sort (`timestamp` + `asc`), pagination still works as "page from the newest window, then return the selected window in chronological order."
- When `next_page` is `true`, the CLI populates `next_command` with a continuation command that preserves active `--source`, `--session`, `--project`, `--all`, `--limit`, `--offset`, `--sort-by`, `--order`, and `--pretty` settings.
- `messages --session <id>` without `--project` bypasses cwd auto-discovery and searches all projects; when `--source` is also omitted, the CLI prints a `stderr` hint encouraging `--source` for narrower lookup.
...
- `remember` writes Markdown by default (`-O md`); use `-O json` for structured output.
- `--model` currently affects Gemini and Cursor backends; Codex ignores it.
- `remember` defaults its project to the raw `current_dir()` string, unlike the canonicalized cwd lookup used by `sessions`, `messages`, and `export`. Use `--project` explicitly when symlink or path-spelling differences matter.
