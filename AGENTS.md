...
- `cargo run -- messages --session sess-123` — messages for a specific session across all projects when `--project` is omitted; prints a stderr hint suggesting `--source` when the source is also omitted.
...
- `mmr messages --session <id>` is the exception: when `--project` is omitted, it bypasses cwd auto-discovery and searches all projects instead because session IDs are treated as global lookups. If `--source` is also omitted, the CLI prints `hint: searching all sources for session; pass --source to narrow the search` on stderr.
- `mmr messages --session <id> --project <path>` still honors the explicit project scope; the bypass applies only when `--session` is present and `--project` is absent.
