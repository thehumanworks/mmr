...
The repository already had one cwd-aware behavior in `export`, which infers the project from the current working directory using the canonical path for Codex, Grok, and Pi, and the slash-to-hyphen form for Claude and Cursor. `remember` also already defaults its project from cwd. `sessions` and `messages` were the remaining commands that still required an explicit `--project` to feel project-local.
...
- Codex, Grok, and Pi matching use the canonical cwd path.
- Claude and Cursor matching use the slash-to-hyphen project name derived from the same cwd.
...
- `MMR_DEFAULT_SOURCE=codex|claude|cursor|grok|pi` supplies the default source filter when `--source` is omitted.
