...
- `messages --session <id>` without `--project` remains the exception: it skips cwd auto-discovery and searches across all projects instead, because session IDs are treated as direct lookups. If `--source` is omitted, the CLI prints a stderr hint suggesting `--source` to narrow the search.
...
- Session-specific lookups remain global by default unless `--project` is supplied, preventing false empty results when the requested session belongs to a different project than the current working directory.
