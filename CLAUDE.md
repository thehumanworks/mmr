...
that document and keep its `status` current (`draft` → `ready` → `in-progress` →
`done`, or `exited`). Deliver goal docs for user review; pointing the agent at a
goal file approves execution. See `goals/` for examples, and `AGENTS.md` for
