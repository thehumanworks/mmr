...
- Decision: Apply `MMR_DEFAULT_SOURCE` to the CLI-level default source only when `--source` is absent, and accept `claude`, `codex`, `cursor`, `grok`, `pi`, or empty.
...
After the tests are in place, update `src/cli.rs`. Introduce small helper functions that read the environment variables, validate them, and resolve the effective defaults. The helper for `MMR_DEFAULT_SOURCE` should parse `claude`, `codex`, `cursor`, `grok`, `pi`, and empty string. The helper for `MMR_DEFAULT_REMEMBER_AGENT` should parse `cursor`, `codex`, `gemini`, and empty string. The helper for project auto-discovery should apply only to `sessions` and `messages`, should return `None` when the user provided `--project` or `--all`, should respect `MMR_AUTO_DISCOVER_PROJECT=0` by disabling discovery, should treat unset or `1` as enabled, and should use the existing cwd canonicalization rules already present in `resolve_project_from_cwd`.
