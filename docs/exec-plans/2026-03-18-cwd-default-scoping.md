...
- Decision: Apply `MMR_DEFAULT_SOURCE` to the CLI-level default source only when `--source` is absent, and treat invalid or empty values as unset.
...
`mmr` is a Rust command-line tool that loads Claude, Codex, Cursor, Grok, and Pi conversation history into in-memory message, session, and project aggregates. The command-line surface lives in `src/cli.rs`. The query and filtering engine lives in `src/messages/service.rs`. Public response shapes and enums live in `src/types/`. Integration tests that exercise the built CLI against a temporary `HOME` tree live in `tests/cli_contract.rs`, with fixture helpers in `tests/common/mod.rs`.

For this task, “auto-discovered project” means deriving a project identifier from the current working directory. This repository already does that for `export`: Codex, Grok, and Pi matching use the canonical cwd path, while Claude and Cursor use the same path encoded with slashes replaced by hyphens and a leading hyphen. `remember` already uses cwd as its default project path. “Scoped by default” means that `sessions` and `messages` must behave as if `--project <cwd-derived-project>` had been supplied when the user did not pass `--project`, when `--all` is absent, and when `MMR_AUTO_DISCOVER_PROJECT` does not disable the feature.
...
After the tests are in place, update `src/cli.rs`. Introduce small helper functions that read the environment variables, validate them, and resolve the effective defaults. The helper for `MMR_DEFAULT_SOURCE` should parse all supported source values and treat empty or invalid strings as unset. The helper for `MMR_DEFAULT_REMEMBER_AGENT` should parse `cursor`, `codex`, `gemini`, and empty string. The helper for project auto-discovery should apply only to `sessions` and `messages`, should return `None` when the user provided `--project` or `--all`, should respect `MMR_AUTO_DISCOVER_PROJECT=0` by disabling discovery, should treat unset or `1` as enabled, and should use the existing cwd normalization rules already present in `resolve_project_from_cwd`.
...
`sessions` and `messages` now auto-discover the cwd project by default, unless `--project` is provided, `--all` is set, or `MMR_AUTO_DISCOVER_PROJECT=0` disables that behavior.

`export` uses cwd-derived source-specific project identifiers when `--project` is omitted: canonical paths for Codex, Grok, and Pi, and slash-to-hyphen names for Claude and Cursor.
...
Revision note: 2026-05-18 / Cursor Automation. Refreshed historical wording so this living document no longer claims a two-source CLI. The chronology is unchanged, but current-state descriptions now reflect the five supported sources (`claude`, `codex`, `cursor`, `grok`, `pi`) and the modern env/default behavior.
