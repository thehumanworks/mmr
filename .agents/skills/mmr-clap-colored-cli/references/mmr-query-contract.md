...
`next_command` is present only when another page exists. It preserves the current `messages` query shape, including `--source`, `--project`, `--all`, `--sort-by`, and `--order` when applicable.
