...
```rust
#[derive(Parser, Debug)]
#[command(name = "mmr", about = "Browse AI conversation history from Claude, Codex, and Cursor")]
...
`SortOrder` and `SourceFilter` follow the same `ValueEnum` + kebab-case pattern:
