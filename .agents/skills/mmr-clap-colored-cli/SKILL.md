...
3. Preserve `mmr` contract semantics:
   ...
   - `sessions` and `messages` auto-discover the cwd project unless `--project` is given, `--all` is set, or auto-discovery is disabled
   - `messages --session <id>` bypasses cwd project auto-discovery when `--project` is omitted
